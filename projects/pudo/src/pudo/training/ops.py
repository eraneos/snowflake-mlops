...
def get_model_registry(
    ...
    return Registry(session=session, database_name=db, schema_name=schema, options=options)
...
def register_model(
    ...
    model_name: str,
    version_name: str,
    train_data: DataFrame | Dataset | None = None,
    ...
    with warnings.catch_warnings():  # Ignore warning about relax_versions from model registration
        ...
        mv = registry.log_model(
            ...
            comment=f"{type(model).__name__} for predicting PUDO fill rate.",
...
def update_metrics(session: Session, model: ModelVersion, metrics: dict[str, float]) -> None:
    """
...
def get_prod_model(session: Session, model_name: str, registry: Registry | None = None) -> ModelVersion:
    """
...
def promote_model(session: Session, model: ModelVersion, registry: Registry | None = None) -> None:
    """
