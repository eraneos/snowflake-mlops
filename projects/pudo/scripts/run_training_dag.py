#!/usr/bin/env python3
"""Execute the training DAG without redeploying.

This script runs an already-deployed training DAG by executing the root task.
Use this for running training cycles without redeploying the DAG definition.

Example usage:
    uv run python scripts/run_training_dag.py
    uv run python scripts/run_training_dag.py --no-wait
"""
...
def main():
    """Execute the training DAG."""
    parser = argparse.ArgumentParser(
        "Run Training DAG",
        description="Execute the already-deployed training DAG.",
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
    ...
    logger.info("🚀 Executing Training DAG")
    logger.info("=" * 60)
    ...
    session = get_session()
    session.append_query_tag("training_dag_execute")
    ...
    dag_name = training_config.pipeline.dag_name.lower()
    ...
    try:
        # Execute via SQL (simpler and more reliable)
        ...
        if not args.no_wait:
            ...
            # Use the same monitoring logic as inference DAG
            ...
            if result == "SUCCEEDED":
                ...
                msg = f"Training DAG failed with result {result}"
    ...
    logger.info("=" * 60)
    logger.info("🎉 Training DAG execution complete!")
