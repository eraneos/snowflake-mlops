"""Snowpark session creation for mock_data operations.

Duplicated from hub-side per ADR-0001 / ADR-0003. Mock data does not pin a
schema in the connection; the seed script and CLI commands target schemas
explicitly via `USE SCHEMA` or fully-qualified names.
"""
...
from pudo_data.core.credentials import get_credentials
...
def create_session(schema: str | None = None) -> Session:
